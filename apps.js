...
class IntBuilder extends Task {
	...
	static random(x, y) {
		let randValue = Math.floor(Math.random() * (y - x) + x);
		return randValue;
	}
...
StringBuilder.prototype.multiply = function (n) {
	this.string = this.string.repeat(n);
	return this;
}
...
StringBuilder.prototype.remove = function (n) {
	this.string = this.string.split(n).shift();
	return this;
}

StringBuilder.prototype.sub = function (x, y) {
	this.string = this.string.substring(x, y + 1);
	return this;
}
